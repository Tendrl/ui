...
class EventList extends Component {
    constructor(props) {
        super(props);
        var $stateParams = window.ngDeps.$stateParams,
    ...
    render() {
        ...
        let eventCount = (this.state.eventList.length > 1) ? (this.state.eventList.length) + " Events" : (this.state.eventList.length) + " Event";

        const noEventFoundMsgEl = (
            <center>
        ...
        const noEventFoundEl = (!this.state.isDataLoading && this.state.eventList.length === 0) ? noEventFoundMsgEl : null;

        const tableEl = (
            <div>
                <div className="flex-table list-group list-view-pf list-view-pf-view event-list-table">
                        {this.state.eventList.map(event => <Event {...event} key={event.message_id}/>)}
                </div>
                <div className="row">
        ...
        const eventData = (!this.state.isDataLoading && this.state.eventList.length !== 0) ? tableEl : null;
                        ...
                        <form className="toolbar-pf-actions">
                            ...
                            <div className="toolbar-pf-action-right">
                                <div className="form-group toolbar-pf-find">
                                    <input name="search-text" id="search-text" type="text" className="form-control" placeholder="Search" />
